...
class User extends CI_Controller {
	...
	**************************************************/
		public function delete($fid){
			...
			Upl_files::delete_file($fid); 	//			delete file on server and delete data from upl_files and file_keys 
	...
	**************************************************/
		...
		/**************	Obtain fuzzy set	****************/
			...
			foreach($keys as $row){
				...
				$ngrams=Ngram::create_enc_ngrams($row);  // No databse query
				foreach($ngrams as $row1){
					$tbnm="index_".$count;
					$count++;
					$tmp_fuzzy_set=Index_x::compare($row1,$tbnm); // databse query
